#!/usr/bin/env bash
# =============================================================================
#  Oh My Zsh — Full Setup Script for Fresh Fedora
...
# ── 1. System packages ───────────────────────────────────────────────────────
info "Updating repositories and installing dependencies via DNF..."
sudo dnf check-update || true
sudo dnf install -y \
  zsh curl git wget unzip fontconfig util-linux-user \
  fzf bat fd-find ripgrep eza \
  python3-pip gcc gcc-c++ make

# Ensure fd is available (Fedora installs fd-find as /usr/bin/fd natively)
...
plugins=(
  ...
  dnf                     # Fedora dnf completions & aliases
  ...
  firewalld               # firewalld helpers (Fedora default)
...
# Load fzf shell integration (Fedora rpm layout)
...
if command -v eza &>/dev/null; then
  ...
  alias llt='eza --tree --level=3 --long --icons --git'
fi
...
alias update='sudo dnf upgrade --refresh -y && sudo dnf autoremove -y'
...
fcd() {
  ...
  dir=$(fd --type d --hidden --follow --exclude .git . "${1:-.}" 2>/dev/null | \
        fzf --preview 'eza -1 --color=always {}' +m) && cd "$dir"
}
